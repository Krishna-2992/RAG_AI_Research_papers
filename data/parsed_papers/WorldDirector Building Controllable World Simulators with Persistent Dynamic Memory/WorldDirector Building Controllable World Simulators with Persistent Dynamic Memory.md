...
Dual-Conditioning Preparation for Dynamic Objects. We construct two conditioning videos for each training sequence. First, to encode spatio-temporal trajectories and provide positional priors, we generate a spatial location condition video by filling each dynamic object's 2D bounding box with a unique color identifier against a zero-initialized background. Second, to ensure appearance consistency for re-entering objects regardless of their absence duration, we introduce an appearance conditioning video. Specifically, for an object a at frame t with bounding box box a,t , we retrieve a reference box a,t ′ of the identical object from the 10-second candidate pool that minimizes aspect ratio divergence relative to box a,t . The image region within box a,t ′ is then cropped, spatially resampled, and mapped onto the coordinates of box a,t in the appearance video, directly equipping the model with exact visual features at designated spatio-temporal indices.

Static and Dynamic Context Retrieval. To better support robust causal inference and preserve spatio-temporal consistency, we retrieve context through a dual-perspective approach. For static scenes, we follow [50] to retrieve the topK frames maximizing Field of View (FoV) overlap. For dynamic objects, we introduce a greedy algorithm prioritizing frames with active object identities within the current temporal chunk. To ensure uniform spatio-temporal distribution, we enforce a minimum temporal stride of four frames between selected frames. Ranked lists derived from both strategies are then interleaved and deduplicated to yield the final N memory frame indices. The detailed selection procedure is outlined in Appendix B.

## 3.2 Building Controllable World Simulator with Persistent Dynamic Memory

Building upon our established data curation pipeline, we present WorldDirector , a framework that reconceptualizes video generation as a controllable world simulation with persistent dynamic memory. We formulate this objective as a conditional denoising process guided by multi-modal structural priors. Formally, let V ∈ R T × 3 × H × W denote a training video sequence comprising T frames. The generative process is conditioned on a composite tuple T = {B , A , P , M} :

- Location Condition B ∈ R T × 3 × H × W encodes the precise spatiotemporal trajectories of 2D bounding boxes for all entities, rendered as identity-preserving, color-coded masks.
- Appearance Condition A ∈ R T × 3 × H × W provides sparse RGB features derived from contextual frames to maintain dynamic object appearance consistency across the sequence.
- Multi-Granularity Prompts P = { p global , p 1 , p 2 , . . . , p k } consists of a global prompt p global summarizing the overall video narrative, coupled with fine-grained textual descriptions { p i } k i =1 detailing the specific semantic behaviors of k dynamic entities.
- Contextual Memory Frames M represents contextual frames retrieved via a dual-stream selection strategy, paired with their corresponding location and appearance conditioning to align feature dimensions with the current generation window, thereby anchoring the generated content within the broader global scene.
...
where z t denotes the noisy latent, and E ( · ) represents the pre-trained 3D VAE encoder. ⊕ denotes channel concatenation. D τ ( · ) formulates our Temporal Drop Mechanism , defined as:

<!-- formula-not-decoded -->

Here, A ( i ) t represents the appearance condition for entity i at global frame t , and k ( i ) ≥ 0 is its instance-specific relative frame index (i.e., the number of frames elapsed since entity i newly entered the view). The full appearance conditioning feature D τ ( A ) aggregates these processed entity-level representations, where masked entities are replaced by null embeddings 0 . The fused latent is then processed by a dedicated layer Conv3D ( · ) , where the channel weights corresponding to E ( D τ ( A ) ) are initialized from the first-frame processor for RGB identity transfer, whereas those for E ( B ) are zero-initialized to learn trajectory-guided generation as a residual process.
...
We follow the flow matching framework [31, 17] to perform post-training using the mean squared error (MSE) loss. The training objective is applied exclusively to the current target segment, while the historical context remains non-noisy and serves solely as a reference. Formally, let x 1 denote the ground-truth latent of the target video chunk and x 0 ∼ N (0 , I ) be the random noise. At a sampled timestep t ∈ [0 , 1] , the training input for the target portion is x tgt,t = tx 1 + (1 -t ) x 0 , with the corresponding ground-truth velocity defined as v t = x 1 -x 0 . As described in Section 3.2, the model u receives a concatenated sequence [ x ctx , x tgt,t ] , where x ctx represents the clean context tokens. The training objective is formulated as:

<!-- formula-not-decoded -->

where Ω = {B , A , P , M} is the union of all location, appearance, text, and contextual conditions, and I tgt denotes the set of token indices belonging to the current video segment. By restricting the loss calculation to I tgt , we ensure that the model learns to synthesize new content anchored by the clean memory of previous frames without attempting to reconstruct the already-determined context. During inference, our method operates in two primary stages as described below: World Planning via LLM and Causal Chunk-Based Generation . Further details regarding the inference implementation are provided in Appendix C.
...
- [18] Daniel Geng, Charles Herrmann, Junhwa Hur, Forrester Cole, Serena Zhang, Tobias Pfaff, Tatiana LopezGuevara, Carl Doersch, Yusuf Aytar, Michael Rubinstein, Chen Sun, Oliver Wang, Andrew Owens, and Deqing Sun. Motion prompting: Controlling video generation with motion trajectories. In Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR) , 2025.
...
```
Algorithm 1 Static and Dynamic Context Retrieval Input: Candidate Context Frames F , Training Frames V , Camera Poses C , 2D Bounding Boxes B , Context Length N Output: Static and Dynamic Context M 1: procedure STATIC_CONTEXT( F , V , C ) 2: for each candidate c ∈ F do 3: score( c ) ← max v ∈V FoV _ Overlap( C c , C v ) 4: end for 5: return F sorted by score( · ) descending 6: end procedure 7: procedure DYNAMIC_CONTEXT( F , B , N, V ) 8: Initialize coverage cnt[ i ] ← 0 for each dynamic entity i appears in V 9: while | selected | < N do 10: Find entity i ∗ = arg min i cnt[ i ] ▷ least-covered entity so far 11: Select frame f ∗ ∈ F with largest area( B ( i ∗ ) f ) ▷ best visible context for entity i ∗ 12: Add f ∗ to selected . Remove f ∗ from F 13: A max ← max e area( B ( e ) f ∗ ) ▷ largest bbox area in f ∗ , used as normalizer 14: for each entity j in f ∗ do 15: cnt[ j ] += area( B ( j ) f ∗ ) / A max ▷ normalized bbox area as visibility weight 16: end for 17: end while 18: return selected 19: end procedure 20: P cam ← STATIC_CONTEXT( F , V , C ) 21: P box ← DYNAMIC_CONTEXT( F , B , N ) 22: M← [ ] 23: for k = 0 , 1 , . . . until |M| = N do 24: Append P cam [ k ] to M if not already in M 25: Append P box [ k ] to M if not already in M 26: end for 27: return M sorted by temporal order
```

In this section, we elaborate on the specific implementation details of the static and dynamic context retrieval mechanism (as outlined in Algorithm 1). This algorithm aims to select N memory frames from a candidate frame set F to construct the final context set M . The inputs primarily include the candidate frames F , the current training frames V , the camera poses C , and the 2D bounding boxes of dynamic entities B . The retrieval process consists of two parallel scoring modules and a subsequent interleaving fusion module:

Static Context Retrieval. The static retrieval module (STATIC\_CONTEXT) aims to find contextual frames with the highest viewpoint overlap to provide comprehensive static background information. For each frame c within the candidate set F , the algorithm calculates the Field of View (FoV) overlap between its camera pose C c and all training frame poses C v , taking the maximum overlap value as the candidate's score. Subsequently, all candidate frames are sorted in descending order based on these scores to generate the static context candidate list P cam .

Dynamic Context Retrieval. To ensure balanced spatio-temporal coverage of dynamic objects, we maintain a coverage counter cnt[ i ] (initialized to 0) for each dynamic entity i ∈ V . In each greedy iteration, we identify the least-covered entity i ∗ and retrieve the frame f ∗ ∈ F that maximizes its visible 2D bounding box area. We then update the coverage for all entities j present in f ∗ by adding their bounding box areas, normalized by the maximum bounding box area A max in f ∗ . This process repeats until sufficient frames are gathered, yielding the dynamic list P box .

Interleaving and Fusion. We alternately append frames from P cam and P box to the final memory set M . During this step, duplicates are discarded, and a minimum temporal stride of four frames is rigidly enforced to guarantee a uniform distribution. Interleaving terminates once |M| = N , and the context frames are returned in chronological order.
...
```
You are an expert with strong 3D spatial imagination capabilities. Given the following information: { "coordinate_system": "OpenGL (X-right, Y-up, Z-backward)", "camera_position": {"position": [0.0, 0.0, 0.0]}, "camera_intrinsics": { "fx": 565.4046, "fy": 565.4046, "cx": 416.0, "cy": 240.0, "image_width": 832, "image_height": 480, "fov_v_deg": 46 }, "ground_height_y": [-1.248], "bboxes_3d": [ { "bbox_3d": { "center": [-1.7175, -0.4237, -3.4715], "dimensions": [0.6799, 1.8, 0.442], "rotation_yaw_deg": 90, "prompt": "A woman walks on the road." } }, {
```

```
"bbox_3d": { "center": [3.227, -0.8014, -7.1893], "dimensions": [1.2749, 1.1321, 3.1872], "rotation_yaw_deg": 90, "prompt": "A car first keep still, then starts driving on the road." } } ] }
```

Here, "coordinate\_system" indicates the 3D coordinate system; "camera\_position" refers to the initial camera location; "camera\_intrinsics" specifies the camera parameters; "ground\_height\_y" is the y-coordinate of the ground; "bboxes\_3d" contains information for multiple subjects. For each "bbox\_3d", "center" represents the initial 3D center coordinates, "dimensions" denotes the actual width, height, and length of the object, and "rotation\_yaw\_deg" is the initial yaw angle. The "prompt" provides the textual description for the trajectory generation.
...
Given the complete spatial location sequence B 1: T and camera poses C 1: T , we first partition them into N sequential chunks of size K . The generative process maintains a global continuous video buffer V , which is initialized with the starting reference frame I 0 .

During the iterative generation, the conditioning strategy adapts based on the temporal state. For the first chunk ( n = 1 ), the model extracts the Appearance Condition ( A ) directly from I 0 guided by the Location Condition B , while the historical context memory ( M ) remains empty as there is no preceding temporal information. For all subsequent chunks ( n &gt; 1 ), the framework dynamically constructs A and retrieves the historical Context ( M ) from the previously generated video buffer V . This retrieval mechanism strictly leverages the location constraints B and camera parameters C to fetch precise dynamic entity identities and static background anchors.

Crucially, to guarantee temporal smoothness at the chunk boundaries, the last frame of the current buffer ( V last ) serves as the conditional initial frame ( I start ) for generating the next chunk V ( n ) . The core diffusion model, WorldDirector , processes these multimodal conditions ( B ( n ) , P , A , M , C ( n ) , I start ) to synthesize the current segment. Finally, we append the generated

## Algorithm 2 Causal Chunk-Based Generation

```
Input: Location condition sequence B 1: T , Captions P , Initial reference frame I 0 , Total frames T , Chunk size K , Camera Poses C 1: T Output: Generated continuous video stream V 1: procedure CAUSAL_GENERATION( B 1: T , P , I 0 , T, K, C 1: T ) 2: Partition B 1: T into N = T/K chunks: {B (1) , . . . , B ( N ) } 3: Partition C 1: T into N = T/K chunks: {C (1) , . . . , C ( N ) } 4: Initialize video buffer V ← I 0 5: for n = 1 , 2 , . . . , N do 6: if n = 1 then 7: A ← APPEARANCE_CONDITION_GENERATION( B , I 0 ) 8: M←∅ ▷ no historical context for the first chunk 9: else 10: A ← APPEARANCE_CONDITION_GENERATION( B , V ) 11: M← CONTEXT_RETRIEVAL( B , C , V ) 12: end if 13: I start ← V last ▷ use the last frame of buffer V as initial frame for current chunk generation 14: V ( n ) ← WorldDirector ( B ( n ) , P , A , M , C ( n ) , I start ) 15: V ← V ∪ V ( n ) [1 :] ▷ append generated chunk without first frame to the buffer 16: end for 17: return V 18: end procedure
```
